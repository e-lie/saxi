#!/usr/bin/env node
// Test script: draw a circle of radius 20mm using G2 arc command.
// Usage: node test2.js [--device /dev/ttyUSB0]
// Axes on this machine: GRBL X = vertical, GRBL Y = horizontal
...
// Circle radius 20mm, center at (X0, Y0).
// Start point: (X0, Y20) — 20mm horizontal from home.
// G2 = clockwise arc. I/J = offset from current pos to center.
const R = 20;
...
const COMMANDS = [
  ...
  { cmd: `G1 X0 Y${R} F3000`,           label: `move to circle start (Y=${R}mm)` },
  { cmd: 'G4 P0.2',                     label: 'dwell' },
  { cmd: 'M5',                          label: 'PEN DOWN' },
  { cmd: 'G4 P0.5',                     label: 'wait servo' },
  { cmd: `G2 X0 Y${R} I0 J${-R} F1000`, label: 'draw full circle (CW)' },
  { cmd: 'G4 P0.2',                     label: 'dwell' },
  { cmd: 'M3 S50',                      label: 'PEN UP' },
